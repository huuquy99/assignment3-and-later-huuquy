...
int main(int argc, char *argv[])
{
    ...
    // setsockopt(sockfd, level, optname, optval, optlen): set socket options
    //   sockfd  = socket_fd    : the socket to configure
    //   level   = SOL_SOCKET   : option is at the socket layer (not protocol-specific)
    //   optname = SO_REUSEADDR : allow reuse of local address/port immediately after close
    //   optval  = &opt         : pointer to option value; 1 = enable
    //   optlen  = sizeof(opt)  : size of the option value
